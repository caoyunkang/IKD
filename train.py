...
class IKD:
    ...
    def get_optimizer(self, model):
        return torch.optim.AdamW(model.parameters(), lr=self.lr_rate)
        ...
        for epoch in range(self.n_epoch):
            ...
            if epoch % self.validate_step == 0 and epoch != 0 or epoch > self.n_epoch - 5:
                ...
                for key in pixel_level_rocauc:
                    ...
                    logger.info(f'{self.category}:{epoch}/{self.n_epoch}, {key}: pxl auc: {pixel_level_rocauc[key]:.4f} pro: {pro_auc_score[key]:.4f}')
...
def parse_args():
    ...
    parser.add_argument('--lr_rate', type=float, required=False, default=0.00005)
    ...
    parser.add_argument('--feature_weight', type=float, nargs='+',default=[1., 2.])
